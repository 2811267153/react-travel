...
export type RecommendProductAction =
    ...
    | FetchRecommendProductSuccesAction
    | FetchRecommendProductFallAction
...
export const giveMeDataActionCreateor = (): ThunkAction<void, RootState, undefined, RecommendProductAction> =>async (dispatch, getState) => {
    dispatch(recommendProductActionCreator())
    try {
        const {data} = await axios.get("http://123.56.149.216:8080/api/productCollections")
        dispatch(recommendProductSuccesActionCreator(data))
    } catch (e) {
        if (e instanceof Error) {
            dispatch(recommendProductFallActionCreator(e))
        }
    }
}